...
const ConfirmOrder = () => {
    ...
    const fetchOders = async () => {
      await fetchOrdersByClientId(clientId)
        .then((res) => {
          ...
          if (orderId) {
            const currentorders = responseData?.filter(
  ...
  const validations = () => {
    const missingUploads = orderDetails.some((item, index) => {
      return (
        (item.serviceCategory === "printings" ||
          item.serviceCategory === "frame making") &&
        (!uploadedPhotos[index] || uploadedPhotos[index].length === 0)
      );
    });
    if (missingUploads) {
      triggerNotification(
        "Please upload at least one image for all printing or frame items.",
        "error"
      );
      return;
    }
    const isDeliveryInfoIncomplete = Object.values(deliveryInfo).some(
      (value) => value.trim() === ""
    );

    if (isDeliveryInfoIncomplete) {
      triggerNotification(
        "Please fill in all delivery details before placing the order.",
        "error"
      );
      return;
    }
    if (!isDeliveryInfoIncomplete) {
      if (
        deliveryInfo.sender_phone_number.length < 10 ||
        deliveryInfo.receiver_phone_number.length < 10
      ) {
        triggerNotification(
      ...
      if (
        !/^\d+$/.test(deliveryInfo.sender_phone_number) ||
        !/^\d+$/.test(deliveryInfo.receiver_phone_number)
      ) {
      ...
      if (deliveryInfo.receiver_name.length < 4) {
        triggerNotification(
          "Reciever name must be atleast 4 characters.",
          "error"
        );
        return;
      }
  ...
  const handleSubmit = async (paymentData) => {
    ...
    const deliveryDetails = {
      senderPhoneNumber: deliveryInfo.sender_phone_number,
    ...
    try {
      const deliveryDetailsSave = await createOrderDelivery(
        orderId,
        deliveryDetails
      );
      if (deliveryDetailsSave.status === 201) {
        ...
        if (paymentData.paymentMethod === "Card") {
          navigate("/display-invoice", {
            state: { orderDetails, paymentMethod, deliveryDetails },
          });
        }
        ...
        triggerNotification(
          "Something went wrong when save delivbery detailsl. Try again.",
          "error"
        );
      }
    } catch (err) {
    ...
    <>
      ...
      <div className="confirm-order-container">
            ...
            {orderDetails &&
              orderDetails.map((item, index) => (
                <div className="selected-item" key={item.orderDetailsId}>
                  ...
                  <div>
                    <p>
                      <strong>{item.serviceDetails.serviceName}</strong>
                    </p>
                      ...
                      <div className="upload-section">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (files.length > 5) {
                              triggerNotification(
                                "Maximum 5 images allowed for this item."
                              );
                              return;
                            }
        ...
        <div className="right-section">
          ...
          <button className="submit-btn" onClick={handlePalaceOrder}>
            Place Order
          </button>
        </div>
      </div>
