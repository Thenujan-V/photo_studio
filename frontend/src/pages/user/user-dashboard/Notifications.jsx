      ...
      if (userOrders.status === 200) {
        ...
        for (const [orderId, items] of Object.entries(grouped)) {
          const enrichedItems = await Promise.all(
            items
              .filter((item) =>
                ["awaiting_approval", "reediting", "approved"].includes(
                  item.status
                )
              )
              .map(async (item) => {
                const fetchedPhoto = await fetchEditedPhotos(
                  item.orderDetailsId
                );
      ...
      <div className="notification">
        {orders.length === 0 ? (
          ...
          <div className="notifications-list">
            {Object.values(groupedOrders).every(
                      ...
                      <div key={orderId} className="order-card">
                        ...
                        {items.map((item, index) => (
                          <div
                            ...
                            className="order-item d-flex flex-row justify-content-between gap-3"
                          >
                            <div className="item-left">
                              <p
                                className="fw-bold bg-dark text-center rounded"
                              ...
                              {expanded[orderId] && (
                                <div className="details">
                                  ...
                                  <button
                                    onClick={() =>
                                      openInNewTab(
                                        `${URLForPhotoPath}/${item.editedPhoto?.photoPath}`
                                      )
                                    }
                                    style={{
                                      marginTop: "10px",
                            ...
                            <div className="btn-group d-flex flex-column justify-content-center">
                              <button
                                disabled={["reediting", "approved"].includes(
                                  item.status
                                )}
                                onClick={() =>
                                  handleStatusChange(
