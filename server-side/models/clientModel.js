...
const clientRegister = (clientData, role_id) => {
  ...
  return new Promise((resolve, reject) => {
    db.query(
      sql,
      [username, mail, phone_number, city, password, role_id],
      (err, result) => {
        if (err) reject(err);
        else resolve(result);
      }
...
const adminRegister = (adminData, role_id) => {
  const { admin_name, mail, phone_number, street, city, password } = adminData;
  const sql = `insert into admin (admin_name, mail, phone_number, street, city, password, role_id) 
                    values (?, ?, ?, ?, ?, ?, ?)`;

  return new Promise((resolve, reject) => {
    db.query(
      sql,
      [admin_name, mail, phone_number, street, city, password, role_id],
      (err, result) => {
        if (err) reject(err);
        else resolve(result);
      }
...
const findAdminByMail = async (mail) => {
