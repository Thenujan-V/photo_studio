...
const fetchPhotoShootDetails = () => {
    ...
    const sql = `select 
        ...
        from photoshoot p join sample_photos s on p.id = s.service_id where s.service_category_id = ? group by p.id`

    return new Promise((resolve, reject) => {
        db.query(sql, [serviceCategoryId],
            (err, result) => {
                if(err) reject(err)
                    else resolve(result)
            }
...
const fetchPrintingDetails = () => {
    ...
    const sql = `select 
            ...
            s.service_id as serviceId, 
            JSON_ARRAYAGG(s.file_path) As photoPaths 
            from printings p join sample_photos s on p.id = s.service_id where s.service_category_id = ? group by p.id `
...
const fetchFramesDetails = () => {
    ...
    const sql = `select 
            ...
            JSON_ARRAYAGG(s.file_path) As photoPaths 
            from frames f join sample_photos s on f.id = s.service_id where s.service_category_id = ? group by f.id`
